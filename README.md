...
> 智析录 —— 拍得快、理得清、提醒复习、能打印的错题本。
> 专业级扫描录入（媲美扫描全能王）· AI 知识点分类 · 举一反三 · SM-2 复习规划 · PDF 导出 · Supabase 单账号自动云同步。纯 Android，无自建服务器。
>
...
"- **Supabase 云同步（单账号自动）**：内置唯一共享账号，安装即自动登录、自动同步，无需任何操作；
  错题/知识点/复习进度/举一反三/题库双向同步（updated_at 仲裁），题目图片走 Storage（用户目录隔离）；
  删除传播（墓碑 + 孤儿图清理）保证两端一致；保存/评分后联网自动补同步 + 回前台补同步 + 手动立即同步
...
- **v0.11.0**：终版交付——白屏修复、Supabase 单账号自动同步、成长系统整体下线（三 Tab 终版 IA）、
  多页连拍「拍→裁剪→确认返回」、每日复习提醒通知、知识点人教 2023 级联+搜索、
  备份密钥条件入包+自动备份、深色对比度达标、裁剪几何单测（拖拽回归）
