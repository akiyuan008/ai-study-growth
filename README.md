...
> 智析录 —— AI 学习成长系统：以「**我是不是在变强**」为主线的 AI 错题本 × 成长引擎。
>
...
`成长 | 错题本 | [中央相机键] | 复习 | 设置` —— 相机键嵌入 dock 托架切口，一键直达拍题。

| Tab | 内容 |
|-----|------|
| **成长** | 成长身份卡、三能力趋势（只显示趋势不定级）、今日事实、成长记忆、NextStep 建议（拍题录入/今日复习入口） |
| **错题本** | 题目列表/详情、真实编辑、AI 追问、举一反三、遗忘曲线小图、间隔历史、科目分组统计 |
| **复习** | FSRS 到期队列、图即题干（大图+点按缩放）、三档自评（仍错/模糊/已会）常驻、AI 智能重排（失败回落确定性排序）、多选导出 |
| **相机键** | 应用内相机 + 连拍缩略堆叠、多页队列（调整/删除/重排）、编辑屏（四角拖拽/旋转/自动校准/增强切换） |
| **设置** | AI Provider 配置、AI 调用日志、通知（复习提醒）、云备份、外观（深色模式三态）、数据与关于 |
...
- **AI 复习重排**：到期队列的智能排序，失败自动回落 FSRS 确定性顺序
...
- **本地优先**：Drift (SQLite) 全量本地存储，AI 密钥存 flutter_secure_storage
- **无服务器云备份**：三通道 WebDAV + AES 加密，自动备份与恢复
- **深色模式三态**：跟随系统/浅色/深色，全仓颜色令牌化（硬编码 Color(0x 清零）
...
| 状态管理 | Riverpod |
| 路由 | GoRouter |
| 本地数据库 | Drift (SQLite)，schemaVersion 3 |
